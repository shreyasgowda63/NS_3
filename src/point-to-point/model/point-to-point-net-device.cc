...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  bool result = m_channel->TransmitStart (p, Ptr<PointToPointNetDevice> (this), txTime);
  if (result == false)
    {
      m_phyTxDropTrace (p);
    }
      ...
      if (!m_promiscCallback.IsNull ())
        {
          ...
          m_promiscCallback (Ptr<PointToPointNetDevice> (this), packet, protocol, GetRemote (), GetAddress (), NetDevice::PACKET_HOST);
        }

      m_macRxTrace (originalPacket);
      m_rxCallback (Ptr<PointToPointNetDevice> (this), packet, protocol, GetRemote ());
    }
}
