...
A waveform generator is implemented by the ``WaveformGenerator`` class.
The waveform generator is used to inject noise into a channel and can be used to
test the behavior of a protocol when interference disrupts a transmission signal.
The generator is flexible and capable of transmitting at different power levels
on different bands. A helper class, ``AdvancedWaveformGeneratorHelper``, is also
provided to assist users in setting up waveforms.

Implementation
##############

Each instance of the ``WaveformGenerator`` class is responsible for generating one waveform.
The model contains a list of time slots. Each time slot has a transmission duration
and an instance of a ``SpectrumValue`` class which describes the frequency bands and power
levels that will be transmitted on each frequency.  When the generator starts it will get
the first item in the list and transmit on the specified frequencies for the specified
duration.  Once the transmission is complete, it will move to the next slot and repeat the
process. Once all of the slots have been transmitted the generator will stop transmitting
for a configurable amount of time before starting over again from the first time slot.

Helpers
#######

There are two helpers which can be used to create instances of the ``WaveformGenerator`` class.
The first helper is the ``AdvancedWaveformGeneratorHelper`` class.  This class is used to set
the Channel, NetDevice, Antenna, Bands and power levels for a waveform generator.  The channel
frequencies (bands) can be specified manually using the Bands vector or through a ``SpectrumModel``.
The duration and transmission power level for each band can be specified using a vector of
power spectral density values or using a ``SpectrumValue``.

There is also a ``WaveformGeneratorHelper`` which maintains the original helper API for
creating WaveformGenerator instances with simple waveform patterns.  This helper has been
deprecated and any instances of it should be replaced with the AdvancedWaveformGeneratorHelper
class.

For cases where lots of waveform generators with different patterns are needed, the
``WaveformConfigLoader`` helper class can be used.  This helper class reads a file containing the
descriptions of one or more waveforms patterns and creates a WaveformGenerator class for each
pattern in the file.  The file format can be found in the class documentation for the
``WaveformConfigLoader``

Examples
########

The ``microwave-oven-with-sweep-example`` demonstrates transmitting a waveform
that matches the waveform generated by a microwave oven.  This example
is capable of loading the microwave waveform pattern from a file or configuring
it manually using the helper class.  The waveform pattern config file is named
``5Mhz_microwave_spectrum.conf`` and is located in the spectrum modules
examples directory.

Usage of the WaveformGenerator can also be seen in some of the Wifi test suites like
``wifi-phy-cca`` and ``wifi-phy-ofdma``

Testing
#######

The WaveformConfigLoaderTestSuite contains a number of TestCases to ensure that
the configuration parser is robust.

The WaveformGeneratorTestSuite contains two TestCases which check that the WaveformGenerator
stops transmitting when told to do so using the Stop function.
