                                                  ...
                                                  bool verbose)
{
  ...
  if (!clipping_enabled)
    {
      spectrumChannel->SetAttribute ("EnableSpatialIndexing", BooleanValue (false));
    }
  else
    {
...
main (int argc, char *argv[])
{
  ...
  std::pair<double, std::vector<unsigned int> > vals1 = run(width, total_time, true, clip_range, loss_model, verbose);
  ...
  std::pair<double, std::vector<unsigned int> > vals2 = run(width, total_time, false, clip_range, loss_model, verbose);
