...
/*
 * This example is modeled after a simple parallel P2P example. 
 * The Test creates a dumbbell topology and logically splits it in
 * half.  The left half is placed on logical processor 0 and the right half
 * is placed on logical processor 1.  
 * 
 * A SimpleDistributedChannel is used to connect n4 and n5.
 *
 *
 *                 -------   -------
 *                  RANK 0    RANK 1
...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("SimpleDistributedChannelTwoRanksExample");
...
main (int argc, char *argv[])
{
  ...
  /*
   * Checksums enabled to make packets more digestible by wireshark and other tools.
   */
  ...
  Config::SetDefault ("ns3::OnOffApplication::DataRate", StringValue ("2048"));
  ...
  SimpleDistributedHelper routerLink;
  routerLink.SetChannelAttribute ("Delay", StringValue ("10ms"));
  ...
  if (systemId == 0)
    {
      ...
      clientHelper.SetAttribute
        ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=10]"));
      clientHelper.SetAttribute
