...
TcpSocketCongestionRouter::SendEmptyPacket (uint16_t flags)
{
  ...
  bool withEct = false;
  if (m_tcb->m_ecnMode == TcpSocketState::EcnPp && ((flags == (TcpHeader::SYN | TcpHeader::ACK | TcpHeader::ECE))
...
TcpSocketCongestionRouter::PersistTimeout ()
{
  ...
  // Congestion response for Window probe in ECN++ will be postponed until the data packet is sent out.
