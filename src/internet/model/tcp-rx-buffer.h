...
class TcpRxBuffer : public Object
{
  ...
  private:
    ...
    bool m_gotFin{false};      //!< Did I received FIN packet?
    uint32_t m_size{0}; //!< Number of total data bytes in the buffer, not necessarily contiguous
    uint32_t m_maxBuffer{32768}; //!< Upper bound of the number of data bytes in buffer (RCV.WND)
    uint32_t m_availBytes{0}; //!< Number of bytes available to read, i.e. contiguous block at head
