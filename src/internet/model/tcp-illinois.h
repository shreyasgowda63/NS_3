...
class TcpIllinois : public TcpNewReno
{
  ...
  private:
    ...
    bool m_rttAbove{false};    //!< True when da > d1
    uint8_t m_rttLow{0};       //!< Number of RTTs da has stayed below d1
    double m_alphaMin{0.3};    //!< Minimum alpha threshold
    double m_alphaMax{10.0};   //!< Maximum alpha threshold
    double m_alphaBase{1.0};   //!< Base value of alpha for standard AIMD
    double m_alpha;            //!< Additive increase factor
    double m_betaMin{0.125};   //!< Minimum beta threshold
    double m_betaMax{0.5};     //!< Maximum beta threshold
    double m_betaBase{0.5};    //!< Base value of beta for standard AIMD
    double m_beta;             //!< Multiplicative decrease factor
    uint32_t m_winThresh{15};  //!< Window threshold for adaptive sizing
    uint32_t m_theta{5};       //!< Number of RTTs required before setting alpha to its max
