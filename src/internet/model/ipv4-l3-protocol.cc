...
Ipv4L3Protocol::SetRoutingProtocol (Ptr<Ipv4RoutingProtocol> routingProtocol)
{
  ...
  m_routingProtocol->SetIpv4 (Ptr<Ipv4L3Protocol> (this));
}
...
Ipv4L3Protocol::AddInterface (Ptr<NetDevice> device)
{
  ...
  tc->RegisterProtocolHandler (MakeCallback (&Ipv4L3Protocol::Receive, this),
                               Ipv4L3Protocol::PROT_NUMBER, device);
  tc->RegisterProtocolHandler (MakeCallback (&ArpL3Protocol::Receive, GetObject<ArpL3Protocol> ().get ()),
                               ArpL3Protocol::PROT_NUMBER, device);
                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  if (ipv4Interface->IsUp ())
    {
      m_rxTrace (packet, Ptr<Ipv4L3Protocol> (this), interface);
    }
  else
    {
      ...
      m_dropTrace (ipHeader, packet, DROP_INTERFACE_DOWN, Ptr<Ipv4L3Protocol> (this), interface);
      return;
    }
  ...
  if (!ipHeader.IsChecksumOk ())
    {
      ...
      m_dropTrace (ipHeader, packet, DROP_BAD_CHECKSUM, Ptr<Ipv4L3Protocol> (this), interface);
      return;
    }
  ...
  if (m_enableDpd && ipHeader.GetDestination ().IsMulticast () && UpdateDuplicate (packet, ipHeader))
    {
      ...
      m_dropTrace (ipHeader, packet, DROP_DUPLICATE, Ptr<Ipv4L3Protocol> (this), interface);
      return;
    }
  ...
  if (!m_routingProtocol->RouteInput (packet, ipHeader, device,
                                      ...
                                      ))
    {
      ...
      m_dropTrace (ipHeader, packet, DROP_NO_ROUTE, Ptr<Ipv4L3Protocol> (this), interface);
    }
}
                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  if (newRoute)
  ...
  else
    {
      ...
      m_dropTrace (ipHeader, packet, DROP_NO_ROUTE, Ptr<Ipv4L3Protocol> (this), 0);
      DecreaseIdentification (source, destination, protocol);
    }
}
          ...
          for ( std::list<Ipv4PayloadHeaderPair>::iterator it = listFragments.begin (); it != listFragments.end (); it++ )
            {
              ...
              CallTxTrace (it->second, it->first, Ptr<Ipv4L3Protocol> (this), interface);
              outInterface->Send (it->first, it->second, target);
            }
        }
      else
        {
          CallTxTrace (ipHeader, packet, Ptr<Ipv4L3Protocol> (this), interface);
      ...
      if (ipHeader.GetTtl () == 0)
        {
          ...
          m_dropTrace (header, packet, DROP_TTL_EXPIRED, Ptr<Ipv4L3Protocol> (this), interface);
          return;
        }
...
Ipv4L3Protocol::RouteInputError (Ptr<const Packet> p, const Ipv4Header & ipHeader, Socket::SocketErrno sockErrno)
{
  ...
  m_dropTrace (ipHeader, p, DROP_ROUTE_ERROR, Ptr<Ipv4L3Protocol> (this), 0);
...
Ipv4L3Protocol::HandleFragmentsTimeout (FragmentKey_t key, Ipv4Header & ipHeader, uint32_t iif)
{
  ...
  m_dropTrace (ipHeader, packet, DROP_FRAGMENT_TIMEOUT, Ptr<Ipv4L3Protocol> (this), iif);
