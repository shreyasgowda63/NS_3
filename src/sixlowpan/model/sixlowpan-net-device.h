...
private:
   ...
   * \param [in] meshHdrSize the size of the BC0 header if mesh routing is used or 0.
   * \param [in] bc0HdrSize the size of the BC0 header if mesh routing is used or 0.
   * \param [out] listFragments A reference to the list of the resulting packets, all with the proper headers in place.
   */
  void DoFragmentation (Ptr<Packet> packet, uint32_t origPacketSize, uint32_t origHdrSize, uint32_t meshHdrSize, uint32_t bc0HdrSize,
