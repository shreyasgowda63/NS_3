...
using KDTreeException = std::runtime_error;
...
class kdTree {
    ...
    private:
        ...
        void initialize(size_t n_points, uint8_t a_dim,
            ...
            if(points.size()!=ids.size()) {throw KDTreeException("points.size != ids.size");}
            if(n_points > points.size()) {throw KDTreeException("n_points > points.size");}
        ...
        void swap(size_t i, size_t j) {
            if( i >= m_kdNodes.size()) {throw KDTreeException("i out of range");}
            if( j >= m_kdNodes.size()) {throw KDTreeException("j out of range");}
        ...
        void connect(const size_t _index) {
            if(_index == 0) {throw KDTreeException("index is 0");}
    ...
    public:
        ...
        void adjust_allowed_depth(uint8_t a_depth){
            if(a_depth>100) {throw KDTreeException("depth larger than 100");}
         ...
         * \param a_dim Number of dimensions to copy into our nodes; remaining dimensions,
         *        if any, are set to zero values.
         * \param points The vector of points used to initialize.
         * \param ids The vector of identifiers for the points, which must be the same size
         *        as the points vector.
         */
        void build_kdTree_median_nthelement(size_t n_points, uint8_t a_dim,
                const std::vector<std::vector<KD_Point_Type>>& points,
                const std::vector<KD_ID>& ids) {
            if (0 == n_points)
            {
              throw KDTreeException ("n_points == 0");
            }
            if (a_dim <= 1)
            {
              throw KDTreeException ("a<=1");
            }
            if (points.size () != ids.size ())
            {
              throw KDTreeException ("points.size != ids.size");
            }
            if (n_points > points.size())
            {
              throw KDTreeException ("n_points > points.size");
            }
        ...
        void insert(const std::vector<KD_Point_Type> &_point, const KD_ID &_id) {
             ...
             */
            if(_point.size() != m_dim) {throw KDTreeException("mismatched point size");}
        ...
        void insert(const std::vector<std::vector<KD_Point_Type>> &points, const std::vector<KD_ID> &ids) {
            if(points.empty()) {throw KDTreeException("Points vector empty");}
            if(points.size()!=ids.size()) {throw KDTreeException("points.size != ids.size");}
