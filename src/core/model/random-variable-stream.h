 ...
 * values, it is sometimes useful to specify fixed \c lowerBound and \c upperBound. The bounded
 * version is defined over the interval \f$[a,b]\f$ as:
 *
 *   \f[
 ...
 *
 *   \f[
 *      \langle x | P(x; \alpha, a, b) \rangle = \frac{1}{\alpha} +
 *          \left(\frac{1}{\alpha} + a \right)\exp^{-\alpha a} -
...
class ExponentialRandomVariable : public RandomVariableStream
{
  public:
     ...
     * \deprecated Deprecated since ns-3.43. Use GetLowerBound and GetUpperBound instead.
     */
    NS_DEPRECATED_3_43("Use GetLowerBound and GetUpperBound instead.")
    double GetBound() const;

    /**
     * \brief Set the upper bound of this RNG.
     * \param bound The upper bound value.
     * \deprecated Deprecated since ns-3.43. Use SetLowerBound and SetUpperBound instead.
     */
    NS_DEPRECATED_3_43("Use SetLowerBound and SetUpperBound instead.")
    void SetBound(double bound);

    /**
     * \brief Sets the lower bound on values that can be returned by this RNG stream.
     *
     * This method sets the minimum value that can be generated by the random number generator (RNG)
     * stream.
     * \param lowerBound The new lower bound value. The RNG stream will not generate values below
     * this bound.
     */
    ...
    /**
     * \brief Sets the upper bound on values that can be returned by this RNG stream.
     *
     * This method sets the maximum value that can be generated by the random number generator (RNG)
     * stream.
     * \param upperBound The new upper bound value. The RNG stream will not generate values above
     * this bound.
     */
     ...
     * \param [in] upperBound Lower bound on values returned.
     * \param [in] lowerBound Upper bound on values returned.
     */
    double GetValue(double mean, double lowerBound, double upperBound);

    /**
     * \copydoc GetValue()
     * \param [in] mean Mean value of the unbounded exponential distribution.
     * \param [in] bound Upper bound on values returned.
     * \deprecated Deprecated since ns-3.43. Use GetInteger(uint32_t,uint32_t,uint32_t) instead.
     */
    NS_DEPRECATED_3_43("Use GetInteger(uint32_t,uint32_t,uint32_t) instead.")
    uint32_t GetInteger(uint32_t mean, uint32_t bound);

    /** \copydoc GetValue(double,double,double) */
    uint32_t GetInteger(uint32_t mean, uint32_t lowerBound, uint32_t upperBound);
 ...
 * values, it is sometimes useful to specify fixed \c lowerBound and \c upperBound.
 * The bounded version is defined over the interval \f$ x \in [a, b] \f$ where \f$ a \geq x_m \f$.
...
class ParetoRandomVariable : public RandomVariableStream
{
  public:
     ...
     * values for the mean, the shape, lower bound, and upper bound.
     */
    ...
    /**
     * \brief Set the scale parameter for the Pareto distribution returned by this RNG stream.
     ...
     * \param [in] lowerBound Lower bound on values returned.
     * \param [in] upperBound Upper bound on values returned.
     */
    double GetValue(double scale, double shape, double lowerBound, double upperBound);
     ...
     * \param [in] shape Shape parameter for the Pareto distribution.
     * \param [in] bound Upper bound on values returned.
     * \deprecated Deprecated since ns-3.43. Use GetInteger(uint32_t,uint32_t,uint32_t,uint32_t)
     * instead.
     */
    NS_DEPRECATED_3_43("Use GetInteger(uint32_t,uint32_t,uint32_t,uint32_t) instead.")
    uint32_t GetInteger(uint32_t scale, uint32_t shape, uint32_t bound);

    /** \copydoc GetValue(double,double,double,double) */
    uint32_t GetInteger(uint32_t scale, uint32_t shape, uint32_t lowerBound, uint32_t upperBound);
 ...
 * \par Bounded Distribution
 *
 * Since Weibull distributions can theoretically return unbounded values, it is sometimes useful to
 * specify fixed \c lowerBound and \c upperBound. The bounded version is defined over the interval
 * \f$ x \in [a, b] \f$. Note that in this case the true mean of the distribution is smaller than
...
class WeibullRandomVariable : public RandomVariableStream
{
  public:
     ...
     * values for the scale, shape, lower bound, and upper bound.
     */
 ...
 *
 * The \c Mean and \c Variance parameters defined in this class correspond to those of the unbounded
 * normal distribution.
 *
 ...
 * values, it is sometimes useful to specify fixed bounds. The
 * NormalRandomVariable can be bounded asymmetrically about the mean
 * by specifying separate lower and upper bounds, \f$b_{lower}\f$ and \f$b_{upper}\f$.
 * This confines the values to the interval \f$[b_{lower}, b_{upper}]\f$.
 *
 * The specified \f$b_{lower}\f$ must be less than or equal to \f$b_{upper}\f$, i.e., \f$b_{lower}
 * \leq b_{upper}\f$. Additionally, the \c Mean \f$\mu\f$ can be outside the interval \f$[b_{lower},
 * b_{upper}]\f$, meaning that while the generated values are constrained within the bounds, the
 * mean does not have to fall within the specified bounds.
 *
 * \par Example
 *
 * Here is an example of how to use this class:
...
class NormalRandomVariable : public RandomVariableStream
{
  public:
     ...
     * values for the mean, variance, lower bound, and upper bound.
     */
    ...
    /**
     * \brief Sets the symmetric bound on the values that can be returned by the RNG stream.
     *
     * This method establishes a symmetric range centered around the mean for the values produced by
     * the random number generator (RNG). The values generated will fall within this symmetric
     * interval.
     *
     * \param bound The new symmetric bound value. The RNG stream will generate values within the
     * interval defined by this bound.
     * \deprecated Deprecated since ns-3.43. Use SetLowerBound and SetUpperBound instead.
     */
    NS_DEPRECATED_3_43("Use SetLowerBound and SetUpperBound instead.")
    void SetBound(double bound);
    ...
    /**
     * \brief Returns the symmetric bound on values that can be returned by this RNG stream.
     * \return The symmetric bound on values that can be returned by this RNG stream.
     * \deprecated Deprecated since ns-3.43. Use GetLowerBound and GetUpperBound instead.
     */
    NS_DEPRECATED_3_43("Use GetLowerBound and GetUpperBound instead.")
    double GetBound() const;
    ...
    double GetValue(double mean,
                    double variance,
                    double lowerBound = -NormalRandomVariable::INFINITE_VALUE,
                    double upperBound = NormalRandomVariable::INFINITE_VALUE);
    ...
    /** \copydoc GetValue(double,double,double,double) */
    uint32_t GetInteger(uint32_t mean, uint32_t variance, uint32_t lowerBound, uint32_t upperBound);
 ...
 * \par Bounded Distribution
 *
 * The Laplacian distribution can be bounded asymmetrically about the location
 * by specifying separate lower and upper bounds, \f$b_{lower}\f$ and \f$b_{upper}\f$.
 * This confines the values to the interval \f$[b_{lower}, b_{upper}]\f$.
 *
 * The specified \f$b_{lower}\f$ must be less than or equal to \f$b_{upper}\f$, i.e., \f$b_{lower}
 * \leq b_{upper}\f$. Additionally, the \c Location \f$\mu\f$ can be outside the interval
 * \f$[b_{lower}, b_{upper}]\f$, meaning that while the generated values are constrained within the
 * bounds, the location does not have to fall within the specified bounds.
 *
 * In this case, the true mean and variance of the distribution are generally smaller than the
 * nominal mean and variance. Only if the bound is symmetric around the nominal mean is the nominal
 * mean preserved.
 */
class LaplacianRandomVariable : public RandomVariableStream
{
  public:
     ...
     * values for the location, the scale, lower bound, and upper bound.
     */
     ...
     * \brief Sets the symmetric bound on the values that can be returned by the RNG stream.
     *
     * This method establishes a symmetric range centered around the location for the values
     * produced by the random number generator (RNG). The values generated will fall within this
     * symmetric interval.
     *
     * \param bound The new symmetric bound value. The RNG stream will generate values within the
     * interval defined by this bound.
     ...
     * \param [in] upperBound upper bound on values returned.
     */
    double GetValue(double location, double scale, double lowerBound, double upperBound);
    ...
    /** \copydoc GetValue(double,double,double,double) */
    uint32_t GetInteger(uint32_t location,
                        uint32_t scale,
                        uint32_t lowerBound,
                        uint32_t upperBound);
