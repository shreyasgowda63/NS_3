                          ...
                          Mac48Address to, Mac48Address from)
{
  ...
  if (packetType != NetDevice::PACKET_OTHERHOST)
    {
      m_rxCallback (Ptr<SimpleNetDevice> (this), packet, protocol, from);
    }

  if (!m_promiscCallback.IsNull ())
    {
      m_promiscCallback (Ptr<SimpleNetDevice> (this), packet, protocol, from, to, packetType);
    }
}
...
SimpleNetDevice::SetChannel (Ptr<SimpleChannel> channel)
{
  ...
  m_channel = channel;
  m_channel->Add (Ptr<SimpleNetDevice> (this));
  m_linkUp = true;
...
SimpleNetDevice::FinishTransmission (Ptr<Packet> packet)
{
  ...
  m_channel->Send (packet, proto, dst, src, Ptr<SimpleNetDevice> (this));
