...
Dot11sStack::InstallStack (Ptr<MeshPointDevice> mp)
{
  ...
  //raw pointers to avoid circular Ptr references
  pmp->SetPeerLinkStatusCallback (MakeCallback (&HwmpProtocol::PeerLinkStatus, hwmp.get ()));
  hwmp->SetNeighboursCallback (MakeCallback (&PeerManagementProtocol::GetPeers, pmp.get ()));
  return true;
}
void
