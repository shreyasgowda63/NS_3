...
main (int argc, char *argv[])
{
  ...
  std::cout << "IP Stack in the network: " << ip << std::endl;
  ...
  if (ip == "v4")
  ...
  else if (ip == "v6")
    {
      ...
      PointToPointHelper p2p_2gb200ms, p2p_1gb5ms, p2p_100mb1ms;
      InternetStackHelper stack;
      Ipv6InterfaceContainer ifs;
      Array2D<Ipv6InterfaceContainer> ifs0(nCN, 3);
      Array2D<Ipv6InterfaceContainer> ifs1(nCN, 6);
      Array2D<Ipv6InterfaceContainer> ifs2(nCN, 14);
      Array2D<Ipv6InterfaceContainer> ifs3(nCN, 9);
      Array3D<Ipv6InterfaceContainer> ifs2LAN(nCN, 7, nLANClients);
      Array3D<Ipv6InterfaceContainer> ifs3LAN(nCN, 5, nLANClients);
      ...
      for (int z = 0; z < nCN; ++z) 
        {
          ...
          oss.str ("");
          oss << 2001 + z << ":1:252::";
          address.SetBase (oss.str ().c_str (), Ipv6Prefix (64));
          ifs = address.Assign (ndc0_1);
          // Create Net2
          std::cout << " 2";
          for (int i = 0; i < 14; ++i) 
              ...
              for (int j = 0; j < nLANClients; ++j) 
                {
                  // Sinks
                  PacketSinkHelper sinkHelper ("ns3::TcpSocketFactory",
                                              Inet6SocketAddress (Ipv6Address::GetAny (), 9999));
                  ApplicationContainer sinkApp = sinkHelper.Install (
                      nodes_net2LAN[z][i][j].Get (0));
                  sinkApp.Start (Seconds (0.0));
                  // Sources
                  ...
                  r2 = 10 * urng->GetValue ();
                  OnOffHelper client ("ns3::TcpSocketFactory", Address ());
                  AddressValue remoteAddress (Inet6SocketAddress (
                                                ifs2LAN[z][i][j].GetAddress (0, 1), 9999));
                  client.SetAttribute ("Remote", remoteAddress);
                  ApplicationContainer clientApp;
                  clientApp.Add (client.Install (nodes_net1[x][r1].Get (0)));
                  clientApp.Start (Seconds (r2));
              ...
              for (int j = 0; j < nLANClients; ++j) 
                {
                  ...
                  OnOffHelper client ("ns3::TcpSocketFactory", Address ());
                  AddressValue remoteAddress (Inet6SocketAddress (
                                                ifs3LAN[z][i][j].GetAddress (0, 1), 9999));
                  client.SetAttribute ("Remote", remoteAddress);
                  ApplicationContainer clientApp;
                  clientApp.Add (client.Install (nodes_net1[x][r1].Get (0)));
                  clientApp.Start (Seconds (r2));
