...
{
  return RoutingProtocol::GetTypeId ();
}
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      Simulator::Schedule (Time (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 10))), &Ipv4RoutingProtocol::SendTo, m_ipver4, socket, packet, destination);
    }
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if (m_ipver4->IsMyOwnAddress (rreqHeader.GetDst ()))
    {
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = m_ipver4->FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), m_ipver4->AODV_PORT));
}
  ...
  if (gratRep)
    {
      ...
      Ptr<Socket> socket = m_ipver4->FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), m_ipver4->AODV_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  Ptr<Socket> socket = m_ipver4->FindSocketWithInterfaceAddress (toNeighbor.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor,m_ipver4-> AODV_PORT));
}
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  if (m_ipver4->IsMyOwnAddress (rrepHeader.GetOrigin ()))
    {
      ...
      m_ipver4->SendPacketFromQueue (dst, toDst.GetRoute ());
      return;
    }
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      Simulator::Schedule (jitter, &Ipv4RoutingProtocol::SendTo, m_ipver4, socket, packet, destination);
    }
}
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
      Ptr<Socket> socket = m_ipver4->FindSocketWithInterfaceAddress (
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, m_ipver4->AODV_PORT));
        }
    }
}
...
RoutingProtocol::SendRerrMessage (Ptr<Packet> packet, std::vector<Ipv4Address> precursors)
{
  ...
  if (precursors.size () == 1)
    {
      ...
      if (m_routingTable.LookupValidRoute (precursors.front (), toPrecursor))
        {
          Ptr<Socket> socket = m_ipver4->FindSocketWithInterfaceAddress (toPrecursor.GetInterface ());
          NS_ASSERT (socket);
          NS_LOG_LOGIC ("one precursor => unicast RERR to " << toPrecursor.GetDestination () << " from " << toPrecursor.GetInterface ().GetLocal ());
          Simulator::Schedule (Time (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 10))), &Ipv4RoutingProtocol::SendTo, m_ipver4, socket, packet, precursors.front ());
          m_rerrCount++;
        }
      return;
    }
  ...
  for (std::vector<Ipv4InterfaceAddress>::const_iterator i = ifaces.begin (); i != ifaces.end (); ++i)
    {
      Ptr<Socket> socket = m_ipver4->FindSocketWithInterfaceAddress (*i);
      NS_ASSERT (socket);
      ...
      Simulator::Schedule (Time (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 10))), &Ipv4RoutingProtocol::SendTo, m_ipver4, socket, p, destination);
    }
}
...
void
RoutingProtocol::SetIPv4AODVProtocol(Ptr<Ipv4RoutingProtocol> ipv4_agent)
{
  m_ipver4 = ipv4_agent;
}
void 
