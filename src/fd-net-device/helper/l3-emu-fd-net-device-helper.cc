...
#include "l3-emu-fd-net-device-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("L3EmuFdNetDeviceHelper");

#define EMU_MAGIC 65867

L3EmuFdNetDeviceHelper::L3EmuFdNetDeviceHelper ()
{
  FdNetDeviceHelper::SetTypeId ("ns3::L3EmuFdNetDevice");
}

void
L3EmuFdNetDeviceHelper::SetFileDescriptor (Ptr<FdNetDevice> device) const
{
  NS_LOG_LOGIC ("Creating L3 EMU socket");

  if (m_deviceName == "undefined")
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::SetFileDescriptor (): m_deviceName is not set");
    }
  ...
  if (rc == -1)
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::SetFileDescriptor (): Can't get interface index");
    }
  ...
  // For now, assume only IPv4 traffic is supported
  ...
  if (rc == -1)
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::SetFileDescriptor (): Can't bind to specified interface");
    }

  rc = ioctl (fd, SIOCGIFFLAGS, &ifr);
  if (rc == -1)
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::SetFileDescriptor (): Can't get interface flags");
    }
  ...
  // Note that the socket does not need to be in promiscuous mode since we are
  // only receiving IP packets.
...
int
L3EmuFdNetDeviceHelper::CreateFileDescriptor (void) const
{
  ...
  if (sock == -1)
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): Unix socket creation error, errno = " << strerror (errno));
    }
  ...
  if (status == -1)
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): Could not bind(): errno = " << strerror (errno));
    }
  ...
  if (status == -1)
    {
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): Could not getsockname(): errno = " << strerror (errno));
    }
  ...
  if (pid == 0)
    {
      ...
      //
      status = ::execlp (L3_RAW_SOCK_CREATOR,
                         L3_RAW_SOCK_CREATOR,                            // argv[0] (filename)
      ...
      //
      NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): Back from execlp(), status = " <<
  ...
  else
    {
      ...
      if (waited == -1)
        {
          NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): waitpid() fails, errno = " << strerror (errno));
        }
      NS_ASSERT_MSG (pid == waited, "L3EmuFdNetDeviceHelper::CreateFileDescriptor(): pid mismatch");
      ...
      if (WIFEXITED (st))
        {
          ...
          if (exitStatus != 0)
            {
              NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): socket creator exited normally with status " << exitStatus);
            }
        }
      else
        {
          NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): socket creator exited abnormally");
        }
      ...
      if (bytesRead != sizeof(int))
        {
          NS_FATAL_ERROR ("L3EmuFdNetDeviceHelper::CreateFileDescriptor(): Wrong byte count from socket creator");
        }
