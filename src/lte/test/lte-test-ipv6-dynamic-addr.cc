...
/* *
   * Scenario:  3 UEs, 2 ENBs, 1 Remote Host, UE0<-->ENB0, UE1<-->ENB0, UE2<-->ENB1
                ...
                Radvd on PGW
                Dynamic Address allocated to UE devices
   ...
   * Pass criteria: 1) Every UDP Echo Request and Reply messages sent and received respectively
                    ...
                    4) Advertised Prefix is used to allocate dynamic addresses in UEs
* */
 ...
 */
class LteIpv6DynamicAddressTestCase : public TestCase
{
public:
  LteIpv6DynamicAddressTestCase ();
  virtual ~LteIpv6DynamicAddressTestCase ();
  ...
  /**
   * \brief Configures UDPClients and starts applications.
   * \param ueNodes NodeContainer with UE nodes containing client node.
   */
  void SetClients (NodeContainer ueNodes);
...
LteIpv6DynamicAddressTestCase::LteIpv6DynamicAddressTestCase ()
  : TestCase ("Test IPv6 Dynamic Address Allocation in UEs and routing at LTE")
{
}
...
void LteIpv6DynamicAddressTestCase::SentAtClient (Ptr<const Packet> p, Ptr<Ipv6> ipv6, uint32_t interface)
{
...
void LteIpv6DynamicAddressTestCase::ReceivedAtClient (Ptr<const Packet> p, Ptr<Ipv6> ipv6, uint32_t interface)
{
  Ipv6Header ipv6Header;
  p->PeekHeader (ipv6Header);
  if (ipv6Header.GetNextHeader () == UdpL4Protocol::PROT_NUMBER)
  ...
  else if( ipv6Header.GetNextHeader () == Icmpv4L4Protocol::PROT_NUMBER)
    {
      //copy RA
    }
}

void LteIpv6DynamicAddressTestCase::EnbToPgw (Ptr<Packet> p)
{
  Ipv6Header ipv6Header;
  p->PeekHeader (ipv6Header);
  if (ipv6Header.GetNextHeader () == UdpL4Protocol::PROT_NUMBER)
...
void LteIpv6DynamicAddressTestCase::TunToPgw (Ptr<Packet> p)
{
  Ipv6Header ipv6Header;
  p->PeekHeader (ipv6Header);
  if (ipv6Header.GetNextHeader () == UdpL4Protocol::PROT_NUMBER)
...
void LteIpv6DynamicAddressTestCase::Checker ()
{
  bool b = false;
  bool check = true;
...
void
LteIpv6DynamicAddressTestCase::SetClients(NodeContainer ueNodes)
{
...
void
LteIpv6DynamicAddressTestCase::DoRun (void)
{
  ...
  m_ueIpIface = ipv6h.AssignWithoutAddress(ueLteDevs);

  RadvdHelper radvdHelper;

  radvdHelper.AddAnnouncedPrefix (pgw->GetDevice(1)->GetIfIndex(), Ipv6Address("7777:f00d::"), 64);

  ApplicationContainer radvdApps = radvdHelper.Install (pgw);
  radvdApps.Start (Seconds (2.0));
  ...
  Simulator::Schedule(Time (Seconds(3.0)), &LteIpv6DynamicAddressTestCase::SetClients, this, ueNodes);

  //Set Callback for Client Sent and Received packets
  Ptr<Ipv6L3Protocol> ipL3 = (ueNodes.Get (0))->GetObject<Ipv6L3Protocol> ();
  ipL3->TraceConnectWithoutContext ("Tx", MakeCallback (&LteIpv6DynamicAddressTestCase::SentAtClient, this));
  ipL3->TraceConnectWithoutContext ("Rx", MakeCallback (&LteIpv6DynamicAddressTestCase::ReceivedAtClient, this));
  ...
  appPgw->TraceConnectWithoutContext ("RxFromS1u", MakeCallback (&LteIpv6DynamicAddressTestCase::EnbToPgw, this));
  appPgw->TraceConnectWithoutContext ("RxFromTun", MakeCallback (&LteIpv6DynamicAddressTestCase::TunToPgw, this));

  // do something about receive for neighbour solicitation
  // check for assigned address has proper prefix

  Simulator::Schedule (Time (Seconds (12.0)), &LteIpv6DynamicAddressTestCase::Checker, this);
 ...
 * \brief test suite 1
 */
class LteIpv6DynamicAddressTestSuite : public TestSuite
{
public:
  LteIpv6DynamicAddressTestSuite ();
};

LteIpv6DynamicAddressTestSuite::LteIpv6DynamicAddressTestSuite ()
  : TestSuite ("lte-ipv6-dynamic-address-test", UNIT)
{
  AddTestCase (new LteIpv6DynamicAddressTestCase, TestCase::QUICK);
}

static LteIpv6DynamicAddressTestSuite lteipv6testsuite;
