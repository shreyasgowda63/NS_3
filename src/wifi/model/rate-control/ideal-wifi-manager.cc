            ...
            for (ChannelWidthMhz j = 20; j <= GetPhy()->GetChannelWidth(); j *= 2)
            {
                ...
                if (mode.GetModulationClass() == WIFI_MOD_CLASS_HT)
                {
                    const auto guardInterval =
                        GetShortGuardIntervalSupported() ? NanoSeconds(400) : NanoSeconds(800);
...
IdealWifiManager::DoGetDataTxVector(WifiRemoteStation* st, ChannelWidthMhz allowedWidth)
{
    ...
    else
    {
            ...
            (GetHtSupported(st) || GetStationHe6GhzCapabilities(st->m_state->m_address)))
        {
            for (uint8_t i = 0; i < GetNMcsSupported(station); i++)
            {
                ...
                {
                    guardInterval = std::max(
                        GetShortGuardIntervalSupported(station) ? NanoSeconds(400)
                                                                : NanoSeconds(800),
                        GetShortGuardIntervalSupported() ? NanoSeconds(400) : NanoSeconds(800));
                }
                txVector.SetGuardInterval(guardInterval);
    ...
    {
        guardInterval =
            std::max(GetShortGuardIntervalSupported(station) ? NanoSeconds(400) : NanoSeconds(800),
                     GetShortGuardIntervalSupported() ? NanoSeconds(400) : NanoSeconds(800));
    }
    else
    {
