...
SpectrumWifiPhyFilterTest::RunOne()
{
    ...
    m_ruBands.clear();
    for (MHz_t bw = 160; bw >= 20; bw = bw / 2)
    {
        for (uint16_t i = 0; i < (m_rxChannelWidth / bw); ++i)
        {
            for (unsigned int type = 0; type < 7; type++)
            {
                ...
                {
                    auto subcarrierGroup = HeRu::GetSubcarrierGroup(bw, ruType, index);
                    HeRu::SubcarrierRange subcarrierRange{subcarrierGroup.front().first,
...
SpectrumWifiPhyGetBandTest::DoRun()
{
    ...
    {
        MHz_t guardWidth = contiguous160Mhz ? channelWidth : (channelWidth / 2);
