...
private:
  ...
  bool m_isMarked;         ///< Packet is marked?
};
...
CodelQueueDiscTestItem::CodelQueueDiscTestItem (Ptr<Packet> p, const Address & addr, bool ecnCapable)
  : QueueDiscItem (p, addr, ecnCapable),
    m_ecnCapablePacket (ecnCapable), m_isMarked (false)
...
{
  if (m_ecnCapablePacket && !m_isMarked)
    {
      m_isMarked = true;
      return true;
    }
            ...
            { // In dropping state
              if (currentTime.GetMicroSeconds () < initialDropNext)
              ...
              else if (currentTime.GetMicroSeconds () >= initialDropNext)
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentCeThreshMarkCount, 2, "There should still be 3 CE threshold exceeded marked packet as packets are not ecnCapable");
                }
            }
        }
      else if (testCase == 4)
