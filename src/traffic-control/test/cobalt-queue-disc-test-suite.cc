...
CobaltQueueDiscMarkTest::Dequeue (Ptr<CobaltQueueDisc> queue, uint32_t modeSize, uint32_t testCase)
{
  ...
  /**
   * Enqueue UDP Packets and check if they are getting dropped
...
void
AckFilterUdpEnqueueTest::AddPacket (Ptr<Packet> p, Ptr<CobaltQueueDisc> queue, Ipv4Header hdr)
{
  Address dest;
...
void
AckFilterSackPermittedTest::AddPacket (Ptr<Packet> p, Ptr<CobaltQueueDisc> queue, Ipv4Header hdr)
{
  Address dest;
  Ptr<Ipv4QueueDiscItem> item = Create<Ipv4QueueDiscItem> (p, dest, 0, hdr);
  queue->Enqueue (item);
...
AckFilterSackPermittedTest::DoRun (void)
{
 ...
 /**
   * Enqueue pkt with urg flag and then check if its being dropped
...
void
AckFilterUrgFlagTest::AddPacket (Ptr<Packet> p, Ptr<CobaltQueueDisc> queue, Ipv4Header hdr)
{
  Address dest;
  Ptr<Ipv4QueueDiscItem> item = Create<Ipv4QueueDiscItem> (p, dest, 0, hdr);
  queue->Enqueue (item);
...
void
CobaltBasicSynAckTest::AddPacket (Ptr<Packet> p, Ptr<CobaltQueueDisc> queue, Ipv4Header hdr)
{
  Address dest;
  Ptr<Ipv4QueueDiscItem> item = Create<Ipv4QueueDiscItem> (p, dest, 0, hdr);
  queue->Enqueue (item);
