...
CsmaNetDevice::Attach (Ptr<CsmaChannel> ch)
{
  ...
  m_deviceId = m_channel->Attach (Ptr<CsmaNetDevice> (this));
  ...
  if (!m_promiscRxCallback.IsNull ())
    {
      ...
      m_promiscRxCallback (Ptr<CsmaNetDevice> (this), packet, protocol, header.GetSource (), header.GetDestination (), packetType);
    }
  ...
  if (packetType != PACKET_OTHERHOST)
    {
      ...
      m_rxCallback (Ptr<CsmaNetDevice> (this), packet, protocol, header.GetSource ());
    }
}
