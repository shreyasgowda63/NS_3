...
Ipv6InterfaceContainer iifc;
  ...
  staDevs = wifi.Install (phy, mac, sta);
...
Ipv6Address prefixCS ("2001:db80::");  //create the prefix 

uint32_t indexRouterCS = backbone1Ifs.GetInterfaceIndex (2);  //AR interface (R-CSMA) 

Ptr<RadvdInterface> routerInterfaceCS = Create<RadvdInterface> (indexRouterCS, 1500, 50);
Ptr<RadvdPrefix> routerPrefixCS = Create<RadvdPrefix> (prefixCS, 64, 1.5, 2.0);

routerInterfaceCS->AddPrefix (routerPrefixCS);

radvd2->AddConfiguration (routerInterfaceCS);
