...
//                   mn
//
// It aims to show behaviour when mobile node disconnects from an ar 
// and joins home-link and reconnects with the previous ar
...
int main (int argc, char** argv)
{
  ...
  NodeContainer cnha;
  cnha.Add (nodes.Get (3));
  cnha.Add (nodes.Get (2));
  ...
  iCiH = ipv6.Assign (dCdH);
  iCiH.SetForwarding (0, true);
  iCiH.SetForwarding (1, true);
  iCiH.SetDefaultRouteInAllNodes (0);
  iCiH.SetDefaultRouteInAllNodes (1);
...
rttop->AddNetworkRouteTo (Ipv6Address ("1001:db80::"), Ipv6Prefix (64), Ipv6Address ("3001:db80::200:ff:fe00:4"), 2, 0);
ipv692 = ar.Get(0)->GetObject<Ipv6> ();
rttop = routingHelper.GetStaticRouting (ipv692);
rttop->AddNetworkRouteTo (Ipv6Address ("4001:db80::"), Ipv6Prefix (64), Ipv6Address ("3001:db80::200:ff:fe00:5"), 2, 0);

ApplicationContainer serverApps = echoServer.Install (mn.Get (0));

serverApps.Start (Seconds (4.0));
serverApps.Stop (Seconds (700.0));

UdpEchoClientHelper echoClient (Ipv6Address ("2001:db80::200:ff:fe00:1"), 9);
...
ApplicationContainer clientApps = echoClient.Install (cn.Get (0));
