...
{
  BCache::Entry *bce = m_bCache->Lookup (addr);
  if (bce && bce->GetState () != BCache::Entry::INVALID)
    {
      return true;
    }
  return false;
...
{
  BCache::Entry *bce = m_bCache->LookupSHoa (addr);
  if (bce && bce->GetState () != BCache::Entry::INVALID)
    {
      return true;
    }
...
Ptr<Packet> Mipv6Ha::BuildBA (Ipv6MobilityBindingUpdateHeader bu,Ipv6Address hoa, uint8_t status, uint16_t lifetime, bool extn)
{
      ...
      if (bu.GetFlagA ())
        {
          ...
          Simulator::Schedule (Seconds (1.), &Mipv6Ha::FunctionDadTimeoutForOffLinkAddress, this, interface, ba, dst, homeaddr);
        }
      return 0;
    }
}
...
void Mipv6Ha::FunctionDadTimeoutForOffLinkAddress (Ptr<Ipv6Interface> interface, Ptr<Packet> ba, Ipv6Address src, Ipv6Address homeaddr)
{
  NS_LOG_FUNCTION (this << interface << ba << src << homeaddr);
  if (m_bCache->Lookup (homeaddr)->GetState () != BCache::Entry::INVALID)
    {
      SendReply (ba, src, m_bCache->Lookup (homeaddr)->GetCoa (), 64);
      SetupTunnelAndRouting (m_bCache->Lookup (homeaddr)); // Tunnel will only be setup if entry is valid.
    }
}

void Mipv6Ha::HandleNS (Ptr<Packet> packet, Ptr<Ipv6Interface> interface, Ipv6Address src, Ipv6Address target)
{
  NS_LOG_FUNCTION (this << interface << src << target);
