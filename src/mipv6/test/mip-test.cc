...
private:
  ...
  int d; // False if MN to CN and true if CN to MN
...
void NoHandoffTestCase::Test ()
{
  ...
  std::clog << "pktcounter=" << pktcounter << " pktcounterathandoff=" << pktcounterathandoff << "\n";
  NS_TEST_ASSERT_MSG_GT_OR_EQ (pktcounter, pktcounterathandoff, "Handoff is not successful");
...
{
  std::clog << "Received Packet at tunnel " << p << " " << ih << " " << oh << " " << i << " pktcounter=" << pktcounter;
  pktcounter++;
  if(d == 0)
    {
      if ((ih.GetSourceAddress () == clientaddress) && (ih.GetDestinationAddress () == serveraddress))
        {
          list2.push_back (p->GetUid ());
          Ptr<Mipv6Mn> mipmn = mn->GetObject<Mipv6Mn> ();          
          pkt_tun_recv &= (oh.GetSourceAddress () == (mipmn->GetCoA ())) && (mipmn->IsHomeMatch (oh.GetDestinationAddress ()));
        }
    }
  else
    {
      if ((ih.GetSourceAddress () == clientaddress) && (ih.GetDestinationAddress () == serveraddress))
        {
          ...
          Ptr<Mipv6Mn> mipmn = mn->GetObject<Mipv6Mn> ();
          pkt_tun_recv &= (oh.GetDestinationAddress () == (mipmn->GetCoA ())) && (mipmn->IsHomeMatch (oh.GetSourceAddress ()));
        }
...
{
  std::clog << "Transfer Packet at tunnel " << p << " " << ih << " " << oh << " " << d << " pktcounter=" << pktcounter;
  if(d == 0)
    {
      if ((ih.GetSourceAddress () == clientaddress) && (ih.GetDestinationAddress () == serveraddress))
        {
          ...
          Ptr<Mipv6Mn> mipmn = mn->GetObject<Mipv6Mn> ();
          pkt_tun_sent &= (oh.GetSourceAddress () == (mipmn->GetCoA ())) && (mipmn->IsHomeMatch (oh.GetDestinationAddress ()));
        }
    }
  else
    {
      if ((ih.GetSourceAddress () == clientaddress) && (ih.GetDestinationAddress () == serveraddress))
        {
          ...
          Ptr<Mipv6Mn> mipmn = mn->GetObject<Mipv6Mn> ();
          pkt_tun_sent &= (oh.GetDestinationAddress () == (mipmn->GetCoA ())) && (mipmn->IsHomeMatch (oh.GetSourceAddress ()));
        }
...
{
  std::clog << "Transfer Packet at clinet " << p << " " << interface << " pktcounter=" << pktcounter;
  Ipv6Header h;
  p->PeekHeader (h);
  if ((h.GetSourceAddress () == clientaddress) && (h.GetDestinationAddress () == serveraddress))
    {
...
{
  std::clog << "Receive Packet at server " << p << " " << interface << " pktcounter=" << pktcounter;
  Ipv6Header h;
  p->PeekHeader (h);
  if ((h.GetSourceAddress () == clientaddress) && (h.GetDestinationAddress () == serveraddress))
    {
...
{
  std::clog << "Transfer BU " << packet << " " << src << " " << dst << " pktcounter=" << pktcounter;
...
{
  std::clog << "Receive BA " << packet << " " << src << " " << dst << " pktcounter=" << pktcounter;
...
//Testing whether correct HoA-CoA mapping exist in HA Cache
  ...
  Ptr<Mipv6Mn> mipmn = mn->GetObject<Mipv6Mn> ();
  coa_match_ha &= (mipmn->GetCoA () == (entry->GetCoa ()));
}
