...
int main (int argc, char** argv)
{
  ...
  Ptr<ns3::RadvdPrefix> prefixPtr = *(radvdHelper.GetRadvdInterface (iic1.GetInterfaceIndex (1))->GetPrefixes ().begin ());
  prefixPtr->SetOnLinkFlag (false);
  Ptr<Ipv6L3Protocol> ipv6l3Protocol = r->GetObject<Ipv6L3Protocol> ();
  if (ipv6l3Protocol)
    {
      Ptr<Ipv6StaticRouting> rp = Ipv6RoutingHelper::GetRouting<Ipv6StaticRouting> (ipv6l3Protocol->GetRoutingProtocol ());
      NS_ASSERT (rp);
      rp->AddHostRouteTo ("2001:1::200:ff:fe00:1", iic1.GetAddress (0, 0), ipv6l3Protocol->GetInterfaceForDevice (tmp2.Get (0)));
    }
  ...
  Ptr<OutputStreamWrapper> outStream = Create<OutputStreamWrapper> (&std::cout);
  Ipv6RoutingHelper::PrintRoutingTableAllAt (Seconds (2), outStream);
