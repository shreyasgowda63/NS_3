...
//
// The simulation assumes a configurable number of stationa in an infrastructure network:
//
//  STA     AP
//    *     *
//    |     |
      ...
      for (int channelWidth = 20; channelWidth <= maxChannelWidth; ) //MHz
        {
          for (int gi = 3200; gi >= 800; ) //Nanoseconds
            {
              // must fit in the max TX duration when transmitting at MCS 0 over an RU of 26 tones
